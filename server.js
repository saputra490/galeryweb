...
// API: Unggah Foto Baru dengan Perbaikan Signature & Auto-Reconnect Database
app.post('/api/foto/unggah', authenticateToken, upload.single('photo'), async (req, res) => {
    try {
        ...
        // Memastikan koneksi database aktif pada arsitektur serverless
        if (!db) {
            await connectDB();
            if (!db) return res.status(500).json({ status: 'error', message: 'Database tidak terdeteksi!' });
        }

        // Perbaikan: Hapus opsi folder statis jika tanda tangan digital bentrok di runtime
        ...
        // Mengunggah stream buffer ke Cloudinary
